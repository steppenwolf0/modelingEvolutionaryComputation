...
def reduceDataset(globalIndex, run) :
	...
	if(globalIndex==0):
		dfData = read_csv("../data/data_"+str(globalIndex)+".csv", header=None, sep=',', dtype=float)
		ids=read_csv("../data/features_"+str(globalIndex)+".csv", header=None, sep=',')
	else:
		dfData = read_csv("./run"+str(run)+"/data_"+str(globalIndex)+".csv", header=None, sep=',', dtype=float)
