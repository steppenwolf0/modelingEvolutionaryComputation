...
def loadDataset() :
	...
	dfData = read_csv("./best/data_0.csv", header=None, sep=',', dtype=float)
...
def runFeatureReduce() :
	...
	for originalClassifier, classifierName in classifierList :
		...
		#pd.DataFrame(yNew).to_csv("./best/yNew"+str(classifierIndex)+".csv", header=None, index =None)
		#pd.DataFrame(yTest).to_csv("./best/yTest"+str(classifierIndex)+".csv", header=None, index =None)
		#pd.DataFrame(indexSeq).to_csv("./best/indexSeq"+str(classifierIndex)+".csv", header=None, index =None)
